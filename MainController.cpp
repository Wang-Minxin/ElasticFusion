...
MainController::MainController(int argc, char* argv[])
  ...
  Parse::get().arg(argc, argv, "-g", groundTruthPose);
    ...
    if (!gui->pause->Get() || pangolin::Pushed(*gui->step)) {
      if ((logReader->hasMore() || rewind) && eFusion->getTick() < end) {
        ...
        if(groundTruthPose.size() != 0)
        {
            ...
            std::getline(file, line);
            int n =
                  sscanf(line.c_str(), "%lf %lf %lf %lf %lf %lf %lf",&x, &y, &z, &qx, &qy, &qz, &qw);
              assert(n == 7);
            ...
            // Sophus::SE3d* T_wc_gt = new Sophus::SE3d(pose.cast<double>());
            // eFusion->set_T_wc(*T_wc_gt);
            file.close();
