...
* -g  : Read the first frame ground truth pose as the world frame
...
在我的项目中，我使用TUM数据集并且需要第一帧建立的world坐标系的z轴与真实世界的z轴是一致的。因此在ElasticFusion基础上，增加了读取第一帧的真实位姿作为SLAM中的world坐标系。命令为，

```bash
./ElasticFusion  -l ~/datasets/fr3_longoffice/fr3_longoffice.klg -f -s 2 -g ~/datasets/fr3_longoffice/FirstFramePose.txt  -o
```

其中，-s表示跳过前两帧，将第三帧作为首帧(第三帧含有真实轨迹), -g为第三帧的真实位姿，格式为`tx ty tz qx qy qz qw`，

```
-0.6832 2.6909 1.7373 0.0003 0.8617 -0.5072 -0.0145
```

增加的代码为，

```cpp
if(groundTruthPose.size() != 0)
{
   std::ifstream file;
   std::string line;
   file.open(groundTruthPose.c_str());
   double x, y, z, qx, qy, qz, qw;
   std::getline(file, line);
   int n =sscanf(line.c_str(), "%lf %lf %lf %lf %lf %lf %lf",&x, &y, &z, &qx, &qy, &qz, &qw);
   assert(n == 7);
   ...
   T.block<3,3>(0,0) = R;
   T.block<3,1>(0,3) = t;
   T_wc = new Sophus::SE3d(T.cast<double>());
   file.close();
   groundTruthPose.clear();
}
```
